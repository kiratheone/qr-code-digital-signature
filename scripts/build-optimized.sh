...
build_service() {
    ...
    if [ "$CACHE_FROM_REGISTRY" = true ] && [ -n "$REGISTRY_URL" ]; then
        build_cmd="$build_cmd --cache-from type=registry,ref=${REGISTRY_URL}/${service}:cache"
        build_cmd="$build_cmd --cache-to type=registry,ref=${REGISTRY_URL}/${service}:cache,mode=max"
    else
...
run_security_scan() {
    ...
    for service in "${services[@]}"; do
        if command -v trivy &> /dev/null; then
            ...
            trivy image --exit-code 1 --severity HIGH,CRITICAL "${service}:latest" || log_warn "$service has security vulnerabilities"
            ...
            docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy:latest image --exit-code 1 --severity HIGH,CRITICAL "${service}:latest" || log_warn "$service has security vulnerabilities"
        else
